...
// Funcion que a partir de un array genera el contenido HTML a insertar en la pagina con la informacion del producto
...
// Funcion que genera el carousel con las imagenes del producto 
...
// Funcion que genera la puntuacion (promedio) del producto
const getProductScore = (array) => {
  let average_score = 0;
  if (localStorage.getItem("additional_comements_score")) {
    additional_comments_score = JSON.parse(
...
// Funcion que muestra los comentarios del producto
const showComments = (array) => {
  ...
  array.forEach((element) => {
    ...
    <div class="container border border-2 pt-3 mb-1 px-0">
      <h4 class="text-primary border-bottom border-2 ps-3 pb-2 m-0">${
        element.user
      } - ${getStars(element.score)}</h4>
      <p class="text-dark ps-3 pt-2 bg-white m-0 p-5">${element.description}</p>
      <p class="text-secondary text-end border-top border-2 mb-0 ps-0 pe-3">${
        element.dateTime
      }</p>
    </div>
    `;
...
// Funcion para generar la fecha + el formato de la misma
const getDate = () => {
  let date = new Date();
...
// Funcion para generar las estrellas de los comentarios
...
// Funcion para generar + conseguir las estrellas de los comentarios que el usuario realiza 
const getCommentStars = () => {
  let score = `
  ...
  localStorage.setItem(
    "additional_comements_score",
    JSON.stringify(additional_comments_score)
  );
  return score;
};

// Funcion para cambiar la cantidad de estrellas que el usuario puntua en el comentario
const changeStar = (id) => {
  if (
    document.getElementById("1").classList.contains("bi-star-fill") &&
    !document.getElementById("2").classList.contains("bi-star-fill")
  ) {
...
const addProductToCart = (id, img, name, cost, currency, cant) => {
  ...
  localStorage.getItem("cart_items_number")
    ? localStorage.setItem(
        "cart_items_number",
        Number.parseInt(localStorage.getItem("cart_items_number")) + 1
      )
...
// Evento 'DOMContentLoaded', cuando el contenido DOM carga llama a ciertas funciones iniciales (como mostrar la informacion del producto), etc.
window.addEventListener("DOMContentLoaded", function () {
  window.scrollTo(0, 0);
  getJSONData(PRODUCT_INFO_URL + product_id + EXT_TYPE).then(function (
    resultObj
  ) {
    ...
    getJSONData(PRODUCT_INFO_COMMENTS_URL + product_id + EXT_TYPE).then(
      function (resultObj) {
        if (resultObj.status === "ok") {
...
// Evento 'clicl' al boton de agregar comentario
