...
const convertPriceToCurrency = (currency) => {
    currentCurrency = currentCurrency? currentCurrency == "UYU"? "USD": "UYU" : "UYU";
...
const nextStep = (step) => {
    ...
    switch (step) {
        ...
        if (street_nameInpt.checkValidity() && house_numberInpt.checkValidity() && esqInpt.checkValidity()) {
            adress = street_nameInpt.value+" "+house_numberInpt.value+" Esq. "+esqInpt.value;
            delivery_type = document.getElementById("standar").checked? document.getElementById("standar").value : document.getElementById("express").checked? document.getElementById("express").value: document.getElementById("premium").value;
        ...
        if (cardNumber | bankAccount) {
            ...
            const paymentMethod_step3 = document.getElementById("paymentMethod_step3");
            ...
            paymentMethod_step3.innerText = cardNumber? "Tarjeta de credito: "+cardNumber : "Cuenta bancaria: "+bankAccount;
...
const addPaymentMethod = () => {
    ...
    if ((bankAccount_inpt.value.length == 12) | (cardNumber_inpt.value.length == 12 && securityCode_inpt.value.length == 3 && cardExpiryDate_inpt.value.length == 5) ) {
...
const finishPurchase = (id) => {
    ...
    , 2000)
    setTimeout(function() {window.location.replace("index.html")}, 4000);
}